...
public class PopupCompat {
    ...
    /**
     * 静态Holder类，静态PopupCompat实例对象
     */
    private static class PopupCompatHolder {
        private static final PopupCompat INSTANCE = new PopupCompat();
    }
     ...
     * @return PopupCompat
     */
    public static PopupCompat get() {
        return PopupCompatHolder.INSTANCE;
    }
    ...
    protected <T> T as(Context context, Class<T> tClass) {
        if (tClass == DialogConfig.class) {
            ...
            Preconditions.checkArgument("tClass has to be the right type");
            return null;
        }
    }
