...
public final class PopupWindowDelegate extends BaseDelegate<PopupWindowConfig, PopupWindow> {
    ...
    protected void bindListener() {
        Preconditions.checkNotNull(config().getPopupRootView(), "please init PopupWindow root view");
    ...
    protected void applyParamsToPopup() {
        ...
        //设置Popup进出场动画
        if (config().getAnimStyle() != Utils.NO_RES_ID) {
        ...
        getPopup().setOnDismissListener(() -> {
            if (!config().isDecorateStatusBar()) {
                updateContentViewDimAmount(Utils.ALPHA_0);
            }
        ...
        getPopup().setTouchInterceptor((v, event) -> {
            if (Utils.isOutOfBounds(v, event)) {
     ...
     */
    protected void updateContentViewDimAmount(@FloatRange(from = Utils.ALPHA_0, to = Utils.ALPHA_1) float dimAmount) {
